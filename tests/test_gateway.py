...
class TestApiResponsePassthrough:
    """Gateway always includes raw API response so the model sees everything."""

    def test_success_includes_api_response(self, db_session):
        """Successful submission includes the raw API response."""
        content = "Content for api response passthrough test."
        token = _make_qc_token(db_session, content)

        with patch("moltbook_tools.gateway.requests.post",
                    return_value=_mock_api_success("mb-pass-123")):
            result = submit(
                action_type="post", content=content, session_id="passthrough-1",
                qc_token=token, submolt="test", title="Test",
            )

        assert result["success"] is True
        assert "api_response" in result
        assert result["api_response"]["data"]["id"] == "mb-pass-123"

    def test_error_includes_api_response(self, db_session):
        """Failed submission includes the raw API response."""
        content = "Content for error passthrough test."
        token = _make_qc_token(db_session, content)

        with patch("moltbook_tools.gateway.requests.post",
                    return_value=_mock_api_failure(500, "Server Error")):
            result = submit(
                action_type="post", content=content, session_id="passthrough-2",
                qc_token=token, submolt="test", title="Test",
            )

        assert result["success"] is False
        assert "api_response" in result
        assert result["api_response"]["error"] == "Server Error"

    def test_model_sees_unexpected_fields(self, db_session):
        """Any unexpected fields in API response are visible to the model."""
        content = "Content that gets a surprise response."
        token = _make_qc_token(db_session, content)
        ...
        mock_resp.json.return_value = {
            "success": True,
            "data": {"id": "mb-surprise-1"},
            "something_unexpected": "the model needs to see this",
        }

        with patch("moltbook_tools.gateway.requests.post",
                    return_value=mock_resp):
            result = submit(
                action_type="post", content=content, session_id="passthrough-3",
                qc_token=token, submolt="test", title="Test",
            )

        assert result["success"] is True
        assert result["api_response"]["something_unexpected"] == "the model needs to see this"
